...
def main() -> None:
    ...
    with out_path.open("w", encoding="utf-8") as out_f:  # crea/sovrascrive output
        for file_path in root_dir.rglob("*"):            # scansione ricorsiva
            ...
            # salta file dentro directory .git o img
            if any(part in {".git", "img", "output"} for part in file_path.parts):
                continue

            # evita di copiare lo stesso script e il file di output
            if file_path in {out_path, Path(__file__).resolve()}:
                continue
